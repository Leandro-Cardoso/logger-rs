...
impl Reader {
    ...
    pub fn read_today(&self) -> Result<String, LoggerError> {
        let file = self.current_log_file();
    ...
    pub fn read_file(&self, file_name: impl AsRef<Path>) -> Result<String, LoggerError> {
    ...
    pub fn read_all(&self) -> Result<String, LoggerError> {
        ...
        for file in files {
            content.push_str(&self.read_file_path(file)?);
        }
    ...
    fn read_file_path(&self, path: PathBuf) -> Result<String, LoggerError> {
        fs::read_to_string(path).map_err(|error| LoggerError::ReadFailed(error.to_string()))
    }
